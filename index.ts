...
app.get('/api/reviews_by_user_id/:userId', async (req: Request, res: Response) => {
  const { userId } = req.params;
  const reviews = await getAllUserReviews(userId);
  res.status(200).json(reviews);
});

app.get('/api/reviews_by_story_id/:storyId', async (req: Request, res: Response) => {
...
app.delete('/api/reviews', async (req: Request, res: Response) => {
  const { userId, reviewId } = req.body;
